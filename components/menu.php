<?php
echo '<section id="nav">
    <div class="navwrapper">
    <div class="logo">
                    <div class="logolar"><img src = "./css/logos/logo.png"/></div>
                    <div class="dik"></div>
                    <div class="logolar"><img src = "./css/logos/mathazone.jpg"/></div>
            </div>
            <nav>
                <ul id="ust">
                    <li><a href="./">Ana Sayfa</a></li>
                    <li><a href="./pages/hakkimizda.php">Hakkımızda</a></li>
                    <li><a href="./pages/egitimprogramlarimiz.php">Eğitim Programlarımız</a></li>
                    <li class="dropdown"> <a href="./pages/haber.php" class="dropbtn">Haberler ve Duyurular</a></li>
                    <li><a href="./pages/ataturk.php">Atatürk Köşemiz</a></li>
                    <li><a href="./pages/bizeulasin.php">Bize Ulaşın</a></li>
                    
                </ul>
           
                

                <div class="cubuk"> <img src="data:image/svg+xml;utf8;base64," /> </div>
            </nav>
            <div class="humberger">
            <img src="./image/menu.png" onclick="menuAc()"/>
            </div>
    </div>
</section>
<section id="altMenu" class="displayNone">
<ul>
<li><a href="./">Ana Sayfa</a></li>
<li><a href="./pages/hakkimizda.php">Hakkımızda</a></li>
<li><a href="./pages/egitimprogramlarimiz.php">Eğitim Programlarımız</a></li>
<li> <a href="./pages/haber.php">Haberler ve Duyurular</a></li>
<li><a href="./pages/ataturk.php">Atatürk Köşemiz</a></li>
<li><a href="./pages/bizeulasin.php">Bize Ulaşın</a></li>

</ul>
</section>
';
